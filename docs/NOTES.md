...
### 3.5 音频频谱图为什么每条都一样（2026-09，已修）

**现象**：所有音频消息的频谱条形状**完全相同**。

**根因**：`ui/helpers.ts` 的 `waveBars()` 是一段**写死的伪随机图案**（`Math.sin(i*12.9898)*43758.5453` 固定噪声），
不读任何音频数据 —— 它从一开始就是占位骨架，真实音乐文件多起来才暴露。

**修复**：改成画**真实波形（振幅包络）**：

- 服务端 `wave.ts` 新增 `computePeaks(decoded, buckets = 96)`：把解码后的 PCM 按下标均匀切成 96 个时间桶，
  取每桶**跨声道的绝对值峰值**，按最大值归一化到 0~100（静音段保底 4，视觉上像播放器的细线）；
  桶很大时按步进抽样（≤2048 点/桶），避免长音频卡住事件循环。
- 上传时顺手算：`UploadService.attachWaveform(meta, absPath)` 在音频落盘后调用一次，把 `peaks` / `dur` 写进文件元数据。
  播放走 `/api/stream` 本来就要解码一遍，所以这里**不增加解码次数**，只多 3~7ms 取峰值（实测 55MB WAV：解码 ~200ms + 峰值 7ms）。
- 协议：`FileMeta.peaks`（0~100 整数数组）、`FileMeta.dur`（秒）。
- 前端：`waveBars(peaks, seed)` 有真实峰值就按它画；**没有时回退到「按文件名做种子的确定性伪随机」** ——
  仍是假的，但每条消息各不相同且稳定，不会再「全都一样」。音频卡片同时显示时长（如 `1:57`）。
- 历史消息：启动时 `backfillWaveforms()` 后台扫一遍音频消息，缺 `peaks` 的解码补上并 `updateMessage`（会广播，前端自动换真实波形）；
  物理文件不在或解码失败就跳过，不影响启动。另加兜底接口 `GET /api/peaks/:key`（LRU 缓存）。

**验证**（`_dev/_e2e_wave.mjs`，上传 `TestRes/` 四条真实音频，页面 + 服务端双向核对）：

| 文件 | 时长 | 服务端峰值 | 页面柱子 |
|---|---|---|---|
| Between The Dunes.wav | 2:45 | 96 个 | 与元数据逐项一致 |
| Brain Empty.wav | 1:57 | 96 个 | 与元数据逐项一致 |
| Lo-Fi Beats To Ride Dragons With.wav | 1:21 | 96 个 | 与元数据逐项一致 |
| Observatory and chill 2.wav | 2:29 | 96 个 | 与元数据逐项一致 |

四条**两两不同**（服务端 4 种波形 / 页面 4 种波形）；`_dev/_e2e_backfill.mjs` 验证「抹掉 peaks 后重启 → 自动补回 96 个峰值」。

> 顺带修掉一个隐患：`complete()` / `finalize()` 里「拿不到发送者设备就 `return { msg: undefined }`」会让文件
> **落盘但不建索引、不广播**（脚本上传或无客户端连接时就是黑洞）。现在回退到 `FALLBACK_SENDER`（deviceId `__server__`）。
