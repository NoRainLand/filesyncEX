...
- **音频播放**：服务端解码生成频谱 / 波形，浏览器原生播放（支持拖动 seek）
...
| 接口 | 作用 |
|---|---|
| `/api/wave/:key` | 生成波形峰值数组 → 前端渲染频谱图（桌面柱状 / 移动指示条） |
...
| 库 | 用途 | 选择理由 |
|---|---|---|
| **zod** | 协议 / 配置 schema 校验 | 一个 schema 同时提供 TS 类型（`z.infer`）与运行时校验，前后端共享单一协议来源，杜绝类型漂移 |
...
- **音频解码缓存占内存**：`waveCache`/`streamCache` 是常驻 Map，大量/大体积音频会持续占用内存。
