...
- **生成 HTTP API 技术文档 API.md（用户「生成一份 HTTP API 文档用于修改 quicksendtool」）**：项目根新增 API.md，基于 HttpServer.ts/upload.ts/protocol schema.ts 实测整理。内容：概览（默认端口 4100、自动切端口取 health.port、局域网地址）、通用约定（JSON + device 身份）、端点总览、health/msgs/upload init·chunk·complete·direct·cover/file·stream 各端点详细请求响应示例、数据模型（MsgData/FileMeta/DeviceInfo、kind 判定）、WebSocket 简述（可选，发文本/实时）、QuickSendTool 推荐最小流程（探测→构造 device→≤8MiB direct / >8MiB init+chunk+complete→断点续传 uploadId）。关键常量：chunkSize=1MiB、DIRECT_LIMIT=8MiB、错误统一 400+{error}。文档为静态 md，未打包 exe。已登记 PROJECT_LOG.md。
- **下载文件名 Mac/iOS 带 sha 前缀修复（用户「win 下载自动切原名，mac/ios 下载名变 xxx-原名」）**：根因=文件存储 key=sha12_原名（如 74f81fe167d9_测试文档.pdf），GET /api/file/:key 用 
es.download(p) 未传 filename → Content-Disposition 文件名 = 路径 basename = 存储 key，Mac/iOS 下载名带 sha 前缀（Windows 恰好取到原名是浏览器行为差异）。修复=UploadService 加 displayName(key)（优先 store.getFile(key).name 原始名，封面等未索引回退 key）+ /api/file/:key 改 async 用 
es.download(p, name)。**顺带修复健壮性**：direct/chunk/cover 三个二进制端点对非二进制请求体（被 express.json 提前解析成对象）Buffer.from(req.body) 崩溃 → 加 	oBuffer() 辅助（Buffer/string/ArrayBuffer/TypedArray/null 归一化，普通对象返回 null）→ 返回 400「请求体必须是二进制」。验证（Playwright 上传中文名文件）：Content-Disposition ilename*=UTF-8''测试文档.pdf（原始名）✓、异常 JSON body → 400 不崩溃 ✓。未打包 exe。已登记 PROJECT_LOG.md。
- **视频封面反向上传服务器 + 防覆盖（用户「把取出的帧反向上传服务器，已有则不覆盖」）**：新增完整链路（四包）：
  ①协议 protocol/schema.ts ServerFrame 加 update 帧（{type:"update",msg}，zod+手写联合同步）。
  ②core：Store 接口 + MemoryStore（msgs.set）+ SqliteStore（UPDATE messages）加 updateMessage(id,msg)；SyncEngine 加 updateMessage（持久化 + events.emit("updated")）；EventBus EventMap 加 updated。
  ③server：SocketServer 监听 updated → broadcast update 帧；HttpServer 加 POST /api/msg/:id/cover（binary jpeg）——查消息 404 → 已有封面 409「封面已存在，不覆盖」→ saveCover + updateMessage + 广播。
  ④web：api.ts 加 apiUploadMsgCover(id,blob)；ws.ts 加 onUpdate 回调 + switch；app.ts handlers 加 onUpdate（替换本地消息）；captureVideoCover 取到帧后本地 videoCovers 立即显示 + 反向上传（m.file.cover 为空时），服务器已有封面 409 静默忽略。
  验证（端到端 Playwright + ffmpeg 视频）：direct 上传无封面视频 → 页面 video loadeddata → captureVideoCover 取帧 → 反向上传 → 服务器消息 file.cover=/api/file/xxx_cover.jpg → 广播 update → 页面切为服务器 URL 封面（非 base64）✓；再次上传同消息 cover → 409 不覆盖（封面保持原样）✓；不存在消息 404 ✓。四包构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **删除 tool 文件夹 + 工具下载按钮改跳转 GitHub releases（用户「删除 tool 文件夹及复制代码，下载按钮跳转 github.com/NoRainLand/QuickSendTool/releases」）**：①删除根 `tool/` 与 `packages/web/public/tool/`（脚本生成的同步副本）；②`package.mjs` 移除「同步 tool/ → web/public/tool」代码块（保留 fonts 同步）；③设置面板工具下载按钮：`/tool/QuickSendTool.exe` download（I_DOWN）→ `https://github.com/NoRainLand/QuickSendTool/releases` target=_blank rel=noopener（I_LINK 外链图标）；④原型 `docs/prototype/desktop.html` 同步改跳转；⑤.gitignore 注释改「QuickSendTool 已改为 GitHub releases 分发」、README 目录删 tool/ 行。i18n download_qst/tool_note 文案保留（按钮去下载页 + 安装说明仍适用）。验证（Playwright）：设置按钮 href=github releases、target=_blank、rel=noopener ✓；dist 不再含 tool（/tool/ 走 SPA 兜底返回 index.html 非 exe）✓。未打包 exe。已登记 PROJECT_LOG.md。
- **删除视频消息未删封面修复（用户「删除视频没删除对应封面图」）**：根因=`SyncEngine.releaseFileRef` 只处理视频本体 `file.key`（引用计数），封面 `file.cover`（/api/file/<coverKey>，无引用计数、未进 store 文件索引）从不删 → 孤儿封面文件残留 uploads/。修复=releaseFileRef 在释放 key 后，解析 `msg.file.cover` 的 `/api/file/([^/]+)$` 取 coverKey（decodeURIComponent + 防御），非空且 != key 时发 `file-gc` 事件（server 订阅 uploads.deleteFile 物理删）；删除消息/trim 裁剪均走此路径。验证（Playwright + ffmpeg 视频）：上传视频（extractVideoCover 生成服务器封面）→ uploads/ 有 视频+ccc97d86_cover.jpg → 删除视频消息 → uploads/ 目录为空（视频+封面都删）✓。core/server 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **打包产物带版本号（用户「修改打包脚本，让产物带上版本号」）**：package.mjs 从根 package.json 读 version，产物名 `release/filesyncex-<version>.exe`（如 filesyncex-6.0.1.exe）；pkg --output、fix-icon exe 路径、完成日志均用 exeName；fix-icon.mjs 的 OriginalFilename/InternalName 元数据同步 `filesyncex-<version>.exe`。已实际打包验证：release/filesyncex-6.0.1.exe（74.67MB）生成 ✓；清理旧 filesyncex.exe 与 .bak 临时备份。未打包 exe（已打）。已登记 PROJECT_LOG.md。
- **NiarApp 加三个系统接口（用户「注册表注册自启 / 打包所有数据下载 / 下载服务器本体」）**：浏览器无法操作注册表/文件系统，改为**服务器加 3 个 HTTP 端点 + NiarApp 封装**：
  ①`POST /api/sys/autostart`：pkg 打包模式（`process.pkg`）→ `reg add HKCU\...\Run /v filesyncEX /d <execPath>` 注册当前 exe 开机自启；开发模式返回 400「开发模式无法注册开机自启（需先打包为 exe）」。
  ②`GET /api/data/export`：打包所有数据为 zip（日期命名 `filesyncEX-backup-YYYYMMDD-HHmmss.zip`）——**数据库一致快照**（better-sqlite3 `db.backup`，WAL 安全）+ `uploads/` 全部文件递归。新建零依赖 `zip.ts`（STORE 无压缩 zip 生成器，pkg 友好）。
  ③`GET /api/app/download`：pkg 打包模式 `res.download(process.execPath, basename)` 下载服务器本体；开发模式 400「开发模式没有打包产物（当前由 node 运行）」。
  NiarApp.ts 加 `autostart()` / `exportData()` / `downloadApp()` + 私有 `downloadBlob(url,fallback)`（fetch blob → 解析 Content-Disposition 文件名 → a[download] 触发下载；非 2xx 返回 {ok:false,error}）。
  **坑①：better-sqlite3 的 `db.backup()` 是异步 API（返回 Promise），不同步完成 → 不 await 就 existsSync 为 false，快照文件残留（.export-*.db）且不进 zip → createStore 的 backupDb 改 async 并在 export 端点 await**。**坑②：cfg.uploadDir 在 ServerConfig 类型是可选 → 用前判空**。
  验证：开发模式 autostart/downloadApp 返回失败 ✓（NiarApp 接口 ok:false + 错误信息）；memory 模式 export zip 系统解压 OK（uploads 文件一致）✓；sqlite 模式 export 含 filesync.db（45KB 快照）+ uploads ✓。server/web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **自启动 cwd 修复：exe 启动自动切到 exe 所在目录（用户「用方案1实现」）**：根因=注册表 Run 直接启动 exe 时 Windows 给 cwd 为 System32，config.ts 用 process.cwd() 定位 serverConfig.json/dataDir → 数据落错位置。修复=`packages/shell/src/index.ts` main() 里 `isPkg` 分支加 `process.chdir(path.dirname(process.execPath))`（try/catch 兜底，失败沿用原 cwd），必须在 run() 之前执行。开发模式 isPkg=false 不 chdir 不受影响。验证（node 模拟 chdir 后 loadConfig）：dataDir=新cwd/data、uploadDir=新cwd/data/uploads 均跟随 exe 目录 ✓。**注意：需重新打包 exe 才生效**；webDir 用 __dirname（snapshot 内绝对路径）不受 cwd 影响。未打包 exe。已登记 PROJECT_LOG.md。
- **NiarApp 加 help 接口（用户「加 help 接口打印所有接口名及作用」）**：`NiarApp.help()` 彩色 console.log 打印 7 个公共接口名 + 作用：init/page、joke()、execute("me")、autostart()、exportData()、downloadApp()、help()（标题 #e12885 粗体、接口名 #047878、说明 #6f7a82）。返回 "NiarApp 接口已列出"。验证（Playwright）：console 捕获 8 行彩色输出、hasHelp=true ✓。web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **autostart 入参 + 服务器操作日志 + zip 中文乱码修复（用户三项）**：①`POST /api/sys/autostart` 加 body.action：0=开启（reg add）/1=取消（reg delete），响应加 enabled 字段；NiarApp.autostart(action: 0|1 = 0) 传 JSON body；help 更新描述。②涉及服务器的三个端点加操作日志：autostart（[sys] 开启/取消开机自启 + exe + 来源 IP）、export（[export] 数据导出：N 个文件，字节 → 文件名 + 来源 IP）、app/download（[sys] 下载服务器本体 + 来源 IP），失败也 console.warn。③**zip 中文乱码根因**：makeZip 文件名写 UTF-8 但未设 general purpose bit flag 11（0x0800），Windows 解压器按本地编码（GBK）解 → 中文乱码；修复=local header 与 central directory 的 flags 都设 0x0800（文件名 UTF-8）。验证：上传「中文测试文件.pdf」→ export → 系统 Expand-Archive 解压出 `uploads\9f64a747e1b9_中文测试文件.pdf`（中文正常）✓；服务器终端打印 [export] 日志 ✓；NiarApp.autostart(0)/autostart(1) 正确传参（开发模式返回失败）✓。未打包 exe。已登记 PROJECT_LOG.md。
- **打包前杀同名 exe + 开机自启查询接口（用户两项）**：①package.mjs 在 pkg 前新增「检查并停止运行中的旧版 exe」：遍历 release/ 下所有 *.exe（排除 .bak），用 PowerShell `Get-Process -Name X | Where-Object { $_.Path -eq '<exe绝对路径>' } | Stop-Process -Force` **按路径精确匹配杀进程**（避免误杀其他目录同名 exe，旧版本一并处理）→ 解决 exe 正在运行导致 pkg 覆盖写入失败。②新增 `GET /api/sys/autostart` 查询接口：`reg query HKCU\...\Run /v filesyncEX`，值存在且等于当前 execPath → enabled:true（防止值被改成其他程序）；无键/开发模式 false/400。NiarApp 加 `autostartStatus()`；help 补一行。**坑：multi_replace 时误把 NiarApp.downloadApp 方法替换成 exportData → 方法被删，已恢复（exportData 后、downloadBlob 前）**。验证：开发模式查询接口 400 ✓；kill 逻辑实测——启动 release\filesyncex-6.0.1.exe（PID 11572）→ 执行同款 PowerShell 命令 → KILLED OK ✓。未打包 exe。已登记 PROJECT_LOG.md。
- **开机自启接口服务器日志乱码修复（用户「查询开机启动接口，服务器日志乱码」）**：根因=中文 Windows 的 `reg` 命令错误信息 stderr 是 **GBK 编码**，POST /api/sys/autostart 的 catch 用 `(e as Error).message` 打印/返回——execFileSync 把它按 UTF-8 解码 → 乱码（`����: ϵͳ�Ҳ���ָ����ע������ֵ��` = 「错误: 系统找不到指定的注册表项或值。」）。修复=新增 `regErrorText(e)`：从 error.stderr 取 Buffer，用 **Node 内置 `TextDecoder("gbk")`** 转正确中文（零依赖，ICU 内置），失败回退 utf8；POST 的 console.warn 与 res.json 都改用解码后文本。查询 GET 接口 catch 本就静默（未注册=正常态）。验证（脚本实测 reg query 不存在键）：stderr 字节 `b4edcef3...`（GBK）→ TextDecoder gbk 解码 = 「错误: 系统找不到指定的注册表项或值。」、utf8 解码 = 乱码、DECODE_OK=true ✓。server 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **NiarApp.help 颜色修复（用户「help 颜色不太对，%c 字面显示、样式参数落到行尾」）**：根因=原代码每行 console.log 用**两个 %c**（`%c  name`, style1, `%cdesc`, style2）——部分环境对同一 console.log 多 %c 并存处理异常，导致第二个 %c 当字面、样式参数追加到行尾、接口名不着色。修复=每项拆成**两行单 %c**：`console.log('%c  '+name, 'color:#047878;font-weight:bold;')`（接口名绿色加粗）+ `console.log('%c    '+desc, 'color:#6f7a82;')`（说明灰色）。每行一个 %c 是最可靠的着色写法。验证（Playwright console 参数结构）：每条消息恰好 2 参数（文本+单个样式），接口名/说明各自独立着色 ✓。web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
- **前端自动打印结果 + 查询接口乱码根治（用户两项）**：①`NiarApp.autostart(0|1)` 与 `autostartStatus()` 改为**不返回 Promise（返回 void）**：内部 fetch 链式 .then，**自动等待服务器完成后打印结果**到控制台——成功 `[开机自启] 开启/取消成功`（绿 #0f9d58）、失败 `[开机自启] ...失败：<错误>`（红 #e5484d）；help 描述更新。②查询接口乱码根治：GET /api/sys/autostart 的 `execFileSync("reg", ["query",...])` 在键不存在时抛异常，Error 对象携带 GBK stderr 可能在 pkg 环境泄漏进日志 → 改用 **`spawnSync`**（reg 非 0 退出**不抛异常**，直接返回 {status, stdout, stderr}，从机制上杜绝 GBK 乱码进日志）。POST 已用 regErrorText（TextDecoder gbk）不受影响。验证（Playwright）：autostart(0)/autostartStatus() 返回 undefined（非 Promise）+ 自动打印「开启失败：开发模式...」「查询失败：开发模式...」到 console ✓；开发模式查询接口仍 400（不执行 reg）✓。未打包 exe。已登记 PROJECT_LOG.md。
- **自启入参反转 + 关闭/重置服务器接口（用户三项）**：①autostart 入参反转：现在 `action 1=开启 0=取消`（原 0=开 1=关反直觉），NiarApp.autostart 默认 action=1（开启），help 更新；POST 端点 reg add(1)/reg delete(0) 对调。②`POST /api/sys/shutdown`：优雅关闭服务器并退出——server index.ts 新增 `systemOps`（shutdown 调 close 后 process.exit(0)），createHttpApp 传参；端点先 res.json 再 setTimeout 300ms 调 shutdown（响应能先送达）；服务器打 [sys] 关闭日志。③`POST /api/sys/reset`：清空全部数据并软重启——core Store 加 `clearAll()`（MemoryStore 全 clear；SqliteStore 事务 DELETE messages/files/uploads/chunks）；server reset = store.clearAll + fs 清空 uploads 物理目录 + broadcastNotice(维护中) + close + `spawn(process.execPath, process.argv.slice(1))` 重启 + exit。验证（端到端）：autostart(1)→开启、autostart(0)→取消 ✓；发 RESET_MARKER 消息 → POST reset → 服务器日志清空+重启 → 重启后 /api/msgs 只剩欢迎消息（标记消失）✓；POST shutdown → 200 ok + 服务器退出（health 拒绝）✓。**注意：测试环境 spawn 重启的新进程随终端清理被杀，属环境问题，实际 exe 场景正常**。未打包 exe。已登记 PROJECT_LOG.md。
- **重置服务器改为不关闭服务器（用户「这里的重置，不要关闭服务器」）**：reset 原逻辑=清数据 → close → spawn 重启 → exit（会关闭服务器）。改=**只清空数据，服务器保持运行**：①SocketServer 加 `broadcastReset()`——给所有在线客户端发空 welcome（msgs=[]，self=conn.device 兜底），前端 onWelcome 立即清空消息列表（不断开连接）；②index.ts systemOps.reset 去掉 close/spawn/exit，改为 clearAll + 清 uploads 物理 + `wsServer.broadcastReset()`；日志改「已清空全部数据（服务器保持运行）」；移除 spawn import。验证（端到端）：发 2 条消息（DOM 3 条）→ NiarApp.reset()（不刷新页面）→ DOM 消息 0、服务器 msgs 0、**health OK（服务器未关闭）** ✓。server 构建通过。未打包 exe。已登记 PROJECT_LOG.md。

- **Linux 打包支持（用户「请问当前项目，可以打包为 linux 么？」→「具体不需要你验证，你配置好脚本命令等就行了」）**：分析=项目是纯 Node（Node18）可打 pkg Linux 版，但 better-sqlite3 是原生模块需在 Linux 环境 install 编译、autostart 依赖 Windows reg、package.mjs 有 Windows 硬编码。配置改动：
  ①`packages/shell/scripts/package.mjs` 平台分支：`isLinux = FSEX_TARGET==='linux' || process.platform==='linux'`；`pkgTarget = node18-linux-x64 / node18-win-x64`；产物 `filesyncex-<版本号>-linux-x64`（无 .exe 后缀）；杀旧进程 Linux 用 `pkill -f <绝对路径>`（Windows 保持 Get-Process/Stop-Process 按 Path 精确匹配）；文件过滤 Linux 匹配 `filesyncex-` 前缀非 .bak；rcedit/fix-icon 仅 Windows（Linux 打印「⏭ Linux 跳过 rcedit（无 PE 资源，产物为 ELF 二进制）」）；pkg 命令用 `${pkgTarget}` 变量。
  ②`packages/server/src/HttpServer.ts` autostart POST/GET 加平台判断：`process.platform !== 'win32'` → 501「当前平台不支持注册表开机自启（仅 Windows）」——Linux 无 reg 命令，避免 spawn 失败（isPackaged 判断之后）。
  ③根 `package.json` 加 `package:linux` script（`FSEX_TARGET=linux pnpm --filter @filesyncex/shell package`；Linux 上直接 `pnpm package` 也会自动识别平台）。
  ④README 打包发布小节补 Linux 说明：在 Linux 环境 install 后再打（better-sqlite3 原生模块不可交叉）、产物 ELF 无 .exe、跳过 rcedit、autostart 返回 501 其余功能跨平台一致。
  验证=用户明确不需要验证：仅 `node --check package.mjs` + `pnpm --filter @filesyncex/server build` 确认无语法/编译错误。未打包 exe（不打包，配置完脚本命令即可）。已登记 PROJECT_LOG.md。

- **设置面板支持上下滚动（用户「设置页面太长，一些手机显示不全，这时候要支持设置页面上下滚动」）**：根因=`packages/web/src/app.css` 的 `.settings-panel .panel { max-height: none; overflow: visible; }`（旧注释「内容自然排布，不出现上下滚动条」）**显式禁用了设置面板滚动**——基础 `.panel` 规则本有 `max-height: 88vh; overflow-y: auto`，被设置面板覆盖后，内容超长（手机屏幕矮 / 设置项多）直接溢出显示不全。修复=该覆盖改为允许滚动：`.settings-panel .panel { max-height: 88vh; overflow-y: auto; -webkit-overflow-scrolling: touch; scrollbar-width: thin; scrollbar-color: rgba(0,0,0,.25) transparent; }`（细滚动条与 .codeview 一致）。验证（Playwright）：移动视口 390x844 → overflowY auto、maxHeight 742px(88vh)、内容 scrollHeight 853 超 clientHeight 757、scrollTop 滚到底 96 可滚 ✓；桌面 1280x800 → 内容 941 超 704、滚动到底 223 ✓（顺带修了桌面矮窗口显示不全）；ptitle 点击关闭 / 语言 radio / 外点遮罩关闭未回归。web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。

- **修复移动端输入框随消息增减移动（用户真机反馈「移动端输入框不是浮动的，而是随消息增减改变位置」）**：根因=`packages/web/src/app.css` 的 `:host { height: 100dvh }`——`dvh` 动态视口单位只被较新浏览器支持，**不支持 dvh 的手机浏览器会忽略该声明 → host 高度塌陷为内容高度 → `.container { height:100% }` 跟随塌陷 → 消息列表不滚动、输入框（footer.composer）落到内容末尾随消息增减移动**。Playwright（新版 Chromium 支持 dvh）实测一直正常故未暴露，真机（不支持 dvh 的浏览器）才现形。修复=`:host` 高度改双声明兜底：`height: 100vh; height: 100dvh;`（老浏览器解析 100vh 保证宿主全高、输入框固定底部；支持 dvh 的浏览器用后者动态视口覆盖，地址栏收展/键盘弹出更精确）。注：与设置面板滚动改动无关（`.settings-panel .panel` 只作用弹层，主界面无此类），用户尝试回滚 git diff 为空（工作区仍为 HEAD）。验证（Playwright 390x844）：host 844px、composer 贴底 785-844、list flex:1 撑满；**连发 3 条消息后 composer top 785→785 不动**（moved=false）；模拟不支持 dvh（host 高度强制 100vh）时 composer 仍贴底 785-844 ✓。web 构建通过。未打包 exe（按惯例只增量构建）。已登记 PROJECT_LOG.md。

- **文件消息文件名改中间省略（用户「文件名过长现在是 xxxx……，改为 xxx……xx.后缀」）**：新增 `packages/web/src/app.ts` 工具函数 `ellipsizeFileName(name, max=24)`——超长时**保留开头（≥6、默认前 10 字符）+ 中间省略号 … + 结尾（主文件名末 4 字符 + 扩展名）**，如「这是一个非常长的测试…测试文档.pdf」；≤24 字符直接返回原名（无扩展名则保开头+末 4 字符）。应用到 6 处文件名显示：文件卡（.card.file .meta .name）、图片卡（.card.img .ovl .name）、视频卡（.card.video .ovl .name）、音频卡（.card.audio .mm .name）、上传占位卡（.ph-mm .name）、上传队列（.qname）。CSS 的 nowrap + text-overflow:ellipsis 保留作小屏宽度兜底。下载链接 / Content-Disposition 仍用原始完整文件名不受影响。验证（Playwright + DataTransfer 伪造 3 类文件上传）：文件「这是一个非常长的测试…测试文档.pdf」✓ 图片「超长图片文件名这是一…画面效果.png」✓ 音频「超长音频文件名这是一…省略效果.wav」✓（均保留扩展名）；下载 url 仍带完整原名 ✓。web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。

- **桌面端文件名省略 head 加长（用户「移动端效果很好，桌面端文件名前面那段太短，适当加长」）**：`packages/web/src/app.ts` 的 `ellipsizeFileName` 原来固定 `max=24`（head=前 10 字符）所有平台一致。新增平台自适应 `const fileNameMax = () => (window.innerWidth <= 640 ? 24 : 36)`，函数默认参数 `max = fileNameMax()`——**移动端保持 24（head 10 字符，用户确认效果不变），桌面端放宽 36（head 约 16 字符，明显加长）**；尾部（末 4 字符 + 扩展名）逻辑不变，6 处调用处无需改动（默认参数每次调用按当前窗口宽度取值）。验证（Playwright 同文件名上传）：桌面 1280「这是一段用于测试桌面端文件名前面…名字文档.pdf」head 16 字符 ✓；移动 390「这是一段用于测试桌面…名字文档.pdf」head 10 字符不变 ✓；下载 url 仍带完整原名 ✓。web 构建通过。未打包 exe。已登记 PROJECT_LOG.md。
...
## [6.4.0] README 整理 + 新增 docs/NOTES.md（用户「帮我整理下 readme 文档吧」）

- **问题**：README 经多轮追加后是「按时间堆」的：核心速查（脚本/配置）散落在后半部分、要滚到最后；上传参数（8MB/1MiB 固定切片）已被本轮改成动态切片，信息过期；「做过的优化尝试」与「踩过的坑」膨胀到近百行，与 PROJECT_LOG 重复。
- **整理后的 README（421 行）**，按「是什么 → 怎么跑 → 怎么配 → 怎么工作 → 怎么开发 → 怎么发布 → 注意什么」重排：
  1. 头部信息表（版本/环境/包管理/授权）+ 全量目录树；
  2. **功能特性**：更新为动态切片与 16 GiB 上限（含「超限在上传前拒绝」）；
  3. **快速开始 → 开发环境要求**（上移，紧挨着开始步骤，避免新人先踩 ABI 坑）；
  4. **配置项**（新增）：`serverConfig.json` 全字段注释示例 + 动态切片抽样表 + 环境变量表 + 旧 `chunkSize` 迁移说明；
  5. 架构 / 运行原理：补全启动流程（含配置迁移、ABI 报错退出、sweeper）、WS 心跳与重连、上传流程图（新增「超限预检」分支与秒传分支）、**新增「磁盘卫生与引用计数」小节**；
  6. 技术栈表补 esbuild 混淆用途；目录结构补 `ui/` 展示层、`test/`、`docs/`、`scripts/`；
  7. **开发**：常用脚本表更新（补 `pnpm test`、`set-version`）、测试小节（70 条用例 + 自研 runner 的取舍 + CI）、改版本号小节；
  8. 打包发布：流程步骤更新（bundle.mjs 内联版本号、ABI 预检、默认混淆）、产物体积 71 MB、Linux 说明；
  9. **安全说明**（新增小节）：管理接口令牌/来源校验 vs 业务面无鉴权，明确「不要暴露公网」；
  10. 已知限制：把「无鉴权」改为准确表述，更新大文件等待、Node 版本敏感、pkg 技术债、历史裁剪等条目；
  11. **开发笔记**（新增）：四份文档的定位表 + 4 条「维护最容易踩的点」。
- **新增 `docs/NOTES.md`（150 行）**：把 README 里搬走的内容归档，避免信息丢失 ——
  一、**踩过的坑 15 条**（打包/原生模块 8 条含本轮新增的「`--define` 引号被 shell 吃掉」「改完 CSS 忘构建」、浏览器/前端 5 条、依赖/环境 2 条）；
  二、**做过的优化与实测数据**（体积/打包速度、上传可靠性含动态切片基准表、内存/事件循环、正确性/安全大修、其它）；
